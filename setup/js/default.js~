$(document).ready(function() {
	...
	$(".step_1 input[type='submit']").live("click", function() {
		...
		// remove error paragraphs at first (if times error is shown and error occurs on task, the times error disappears)
		$(".step_1 .cf p").remove();
		...
		var value = $.trim($(this).prev().val().replace(/ +(?= )/g,'')),
			...
			// "subject" for error message ("times" or "task")
			subject = $(this).siblings("input[type='text']").attr("id").split('create_').join(''),
			times_exists = false,
			error_message = "";

		// check if times already exists
		...
		if ( value ) {
			if ( times_exists ) {
				...
				// reset value of input after times or task was created
	...
	$(".step_1 button").live("click", function() {
		...
		// check if times already completed a task
		var times_exists = false,
			task_exists = false;

		// iterate timess
		for ( var i = 0, length = final_result.length; i < length; i++ ) {
			// if times exists
			if ( final_result[i].times === settings[0] ) {
				times_exists = true;
				// iterate tasks
		...
		// if times doesn’t exist make first push to array and continue to step 2
		if ( !times_exists ) {
			final_result.push(
				{
					times: settings[0],
			...
			$(".step_1").hide();
			$(".step_2").show();
			$("<ul class='info'><li><strong>times:</strong> " + settings[1] + "</li><li><strong>Task:</strong> " + settings[3] + "</li></ul>").insertAfter("h2");
		} else {
			// if times didn’t complete the task make push to tasks array and continue to step 2
			if (!task_exists) {
				...
				$(".step_1").hide();
				$(".step_2").show();
				$("<ul class='info'><li><strong>times:</strong> " + settings[1] + "</li><li><strong>Task:</strong> " + settings[3] + "</li></ul>").insertAfter("h2");			} else {
				// if times already did complete the task trough an error
				var error_paragraph = "times <strong>" + settings[1] + "</strong> already accomplished task <strong>" + settings[3] + "</strong>.";
	...
	$(".step_3 button").live("click", function() {
		...
		if ( pairs_length ) {
			...
			tableoutput = "<thead><tr><th>timess</th>";
			...
			for (var i = 0; i < final_result.length; i++) {
				tableoutput += "<tr><th>" + $(".step_1 label[for='" + final_result[i].times + "']").html() + "</th>";
